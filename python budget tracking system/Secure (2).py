...
class Home:
    ...
    def ConfirmPassword(self):
        ...
        for i in range(2,self.sheet.max_row + 1):
            if self.sheet.cell(i,2).value == self.user_name:
                try:
                    if self.sheet.cell(i,3).value == eval(self.user_password):
                except:
                    if self.sheet.cell(i,3).value == self.user_password:               
                        if self.sheet.cell(i,4).value == "Activate":                    
                            allow = True
                            if self.user_name == "IT":
                                ...
                                Nextpage = Department(self.sheet.cell(i,1).value)
                            self.login.destroy()
...
class Department(Home):
    ...
    def ReportDisplay(self):
        ...
        for i in range(1,self.sheet.max_row + 1):
            ...
            for j in range(1,4):
                Label(self.SearchMain,text = self.sheet.cell(row = i, column = j).value,font=("Times New Roman", 13) ,bg = "#CCFFFF").grid(row = i + 2,column = j + 10, pady = 20, padx = 40)
